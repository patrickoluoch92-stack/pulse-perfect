...
export const Route = createFileRoute("/_authenticated/listings/admin/coupons")({
  head: () => ({ meta: authPageMeta({ title: "Coupons", description: "Create and manage platform-wide discount codes." }) }),
...
function CouponsAdmin() {
  ...
  const upsert = useMutation({
    ...
    onSuccess: () => {
      ...
      setOpen(false); setCode(""); setDescription(""); setDiscountValue("10"); setMaxRedemptions(""); setExpiresAt("");
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 p-6">
      <header className="flex items-center justify-between">
        <div>
          <h1 className="flex items-center gap-2 text-2xl font-semibold"><Ticket className="h-6 w-6" /> Coupons</h1>
          <p className="text-sm text-muted-foreground">Discount codes usable at checkout.</p>
        </div>
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild><Button><Plus className="mr-1 h-4 w-4" /> New coupon</Button></DialogTrigger>
          <DialogContent>
            <DialogHeader><DialogTitle>Create coupon</DialogTitle></DialogHeader>
            <div className="space-y-3">
              <div><Label>Code</Label><Input value={code} onChange={(e) => setCode(e.target.value.toUpperCase())} placeholder="SUMMER25" /></div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div><Label>Value</Label><Input type="number" value={discountValue} onChange={(e) => setDiscountValue(e.target.value)} /></div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div><Label>Max redemptions</Label><Input type="number" value={maxRedemptions} onChange={(e) => setMaxRedemptions(e.target.value)} placeholder="Unlimited" /></div>
            ...
            <DialogFooter>
              <Button onClick={() => upsert.mutate()} disabled={!code || upsert.isPending}>Save</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
      </header>
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <Badge variant="secondary">
                    {c.discount_type === "percent" ? `${c.discount_value}% off` : `KES ${c.discount_value} off`}
                  </Badge>
                </div>
                <p className="text-xs text-muted-foreground">
                  Redemptions: {c.redemptions_count}{c.max_redemptions ? ` / ${c.max_redemptions}` : ""}
