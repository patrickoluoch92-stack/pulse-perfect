...
import {
  upsertPrivateOwner,
  getMyPrivateOwner,
  listAcceptingProviders,
  submitVehicleToProvider,
  listMySubmissions,
  withdrawSubmission,
...
const JOURNEY = [
  {
    icon: Building2,
    title: "Choose a rental company",
    desc: "Pick a verified HostPulse partner you'd like to manage your vehicle.",
  },
  {
    icon: Send,
    title: "Submit your vehicle",
    desc: "Share vehicle details, photos and documents in a short application.",
  },
  {
    icon: ClipboardCheck,
    title: "Review & inspection",
    desc: "The company reviews your application and may schedule an inspection.",
  },
  {
    icon: BadgeCheck,
    title: "Company approval",
    desc: "Once approved, your vehicle joins the company's managed fleet.",
  },
  {
    icon: Users,
    title: "Customers book through the company",
    desc: "The company handles pricing, bookings, customers and day-to-day operations.",
  },
  {
    icon: Coins,
    title: "You earn income",
    desc: "You keep ownership and receive payouts on every completed rental.",
  },
] as const;
    ...
    <div className="mx-auto max-w-5xl space-y-10">
      <section className="rounded-2xl border bg-gradient-to-br from-primary/10 via-background to-background p-8 sm:p-12">
        ...
        <div className="mt-6 flex flex-wrap gap-3 text-xs text-muted-foreground">
          <span className="inline-flex items-center gap-1">
            <ShieldCheck className="h-3.5 w-3.5 text-primary" /> Verified partners only
          </span>
          <span className="inline-flex items-center gap-1">
            <Sparkles className="h-3.5 w-3.5 text-primary" /> You retain ownership
          </span>
          <span className="inline-flex items-center gap-1">
            <Coins className="h-3.5 w-3.5 text-primary" /> Transparent payouts
          </span>
        </div>
      </section>
      ...
      <section>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          A simple journey from application to earning.
        </p>
...
function PartnershipApplication({
  ...
  const [p, setP] = useState({
    ...
    preferredPaymentMethod: (existingOwner?.preferred_payment_method ?? "mpesa") as
      | "mpesa"
      | "bank"
      | "both",
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((row) => {
      if (query && !`${row.name} ${row.town ?? ""}`.toLowerCase().includes(query.toLowerCase()))
        return false;
    ...
    <div className="mx-auto max-w-4xl space-y-4">
          ...
          <CardContent className="space-y-4">
            <div className="grid gap-2 sm:grid-cols-[1fr,180px]">
              <div className="relative">
                ...
                <Input
                  className="pl-9"
                  placeholder="Search by company name or town"
                  value={query}
                  onChange={(e) => setQuery(e.target.value)}
                />
              </div>
              <Input
                placeholder="County code (e.g. 47)"
                value={county}
            ...
            {providers.isLoading ? (
              <LoadingState label="Loading partner companies" />
            ) : filtered.length === 0 ? (
              <EmptyState
                title="No partner companies match"
                description="Try broadening your search — more verified companies are joining."
              />
            ) : (
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center gap-3">
              {selected.logo_url ? (
                <img src={selected.logo_url} alt="" className="h-12 w-12 rounded object-cover" />
              ) : (
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  ...
                  {selected.vehicle_count != null
                    ? ` · ${selected.vehicle_count} vehicles under management`
                    : ""}
          ...
          <CardContent className="space-y-4 text-sm">
            <p>
              You're starting a <strong>Vehicle Partnership Application</strong> with{" "}
              {selected.name}. You retain full ownership of your vehicle — {selected.name}'s fleet
              team will handle bookings, customer communication, pricing and day-to-day operations
              once approved.
            </p>
      ...
      {step === "location" && selected && (
        <Card>
          <CardHeader>
            ...
            <p className="text-xs text-muted-foreground">
              Where is your vehicle based? {selected.name} uses this to plan pickups and coverage.
            </p>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <Fld
                label="County *"
                value={loc.countyCode}
                onChange={(x) => setLoc({ ...loc, countyCode: x })}
                placeholder="e.g. 47"
              />
              <Fld
                label="Town / City *"
                value={loc.town}
                onChange={(x) => setLoc({ ...loc, town: x })}
                placeholder="Nairobi"
              />
              <div className="sm:col-span-2">
                <Fld
                  ...
                  onChange={(x) => setLoc({ ...loc, pickupLocation: x })}
                  placeholder="Estate / landmark / street"
                />
              </div>
            ...
            <NavRow
              onBack={() => back(step)}
              onNext={() => next(step)}
              nextDisabled={!loc.countyCode || !loc.town}
            />
          </CardContent>
        </Card>
      )}
      ...
      {step === "about" && selected && (
        <Card>
          <CardHeader>
            ...
            <p className="text-xs text-muted-foreground">
              Your personal contact details. Kept private and only shared with {selected.name}.
            </p>
          </CardHeader>
          <CardContent className="space-y-4">
            <ApplyStepper step={step} />
            <div className="grid gap-3 sm:grid-cols-2">
              <Fld
                label="Full name *"
                value={p.legalName}
                onChange={(x) => setP({ ...p, legalName: x })}
              />
              <Fld
                label="Phone number *"
                value={p.phone}
                onChange={(x) => setP({ ...p, phone: x })}
                placeholder="07XXXXXXXX"
              />
              <Fld
                label="Email address *"
                value={p.email}
                onChange={(x) => setP({ ...p, email: x })}
              />
              <Fld
                label="National ID / Passport *"
                value={p.idNumber}
                onChange={(x) => setP({ ...p, idNumber: x })}
              />
              <div>
              ...
              <Fld
                ...
                onChange={(x) => setP({ ...p, emergencyContact: x })}
            ...
            <NavRow
              onBack={() => back(step)}
              onNext={() => next(step)}
              nextDisabled={!p.legalName || !p.phone || !p.email || !p.idNumber}
            />
          </CardContent>
        </Card>
      )}
      ...
      {step === "vehicle" && selected && (
        <Card>
          <CardHeader>
            ...
            <p className="text-xs text-muted-foreground">
              Essential details about the vehicle you're partnering.
            </p>
          </CardHeader>
          <CardContent className="space-y-4">
            <ApplyStepper step={step} />
            <div className="grid gap-3 sm:grid-cols-3">
              <Fld
                ...
                onChange={(x) => setV({ ...v, registrationNo: x })}
                placeholder="KDA 123A"
              />
              <Fld
                label="Make *"
                value={v.make}
                onChange={(x) => setV({ ...v, make: x })}
                placeholder="Toyota"
              />
              <Fld
                label="Model *"
              ...
              <Fld
                label="Year *"
                value={String(v.year)}
                onChange={(x) => setV({ ...v, year: Number(x) })}
                type="number"
              />
              <Fld
                label="Body type"
                value={v.bodyType}
                onChange={(x) => setV({ ...v, bodyType: x })}
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setV({ ...v, transmission: e.target.value })}
                >
              ...
              <Fld
                ...
                onChange={(x) => setV({ ...v, seats: Number(x) })}
              />
              <Fld
                label="Mileage (km)"
                type="number"
                value={String(v.mileageKm)}
                onChange={(x) => setV({ ...v, mileageKm: Number(x) })}
              />
            </div>
            ...
            <div>
              ...
              <Textarea
                ...
                placeholder="Anything the fleet team should know about your vehicle."
              />
            </div>
            <NavRow
              onBack={() => back(step)}
              onNext={() => next(step)}
              nextDisabled={!v.make || !v.model || !v.registrationNo}
            />
          </CardContent>
        </Card>
      )}
      ...
      {step === "condition" && selected && (
        <Card>
          <CardHeader>
            ...
            <p className="text-xs text-muted-foreground">
              Help {selected.name} understand the state of your vehicle.
            </p>
          </CardHeader>
          <CardContent className="space-y-4">
            <ApplyStepper step={step} />
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                {(["excellent", "good", "fair"] as const).map((c) => (
                  <button
                    key={c}
                    type="button"
                    onClick={() => setCond({ ...cond, overallCondition: c })}
            ...
            <YesNoRow
              label="Is the vehicle insured?"
              value={cond.insured}
              onChange={(b) => setCond({ ...cond, insured: b })}
            />
            <YesNoRow
              label="Is it roadworthy?"
              value={cond.roadworthy}
              onChange={(b) => setCond({ ...cond, roadworthy: b })}
            />
            <YesNoRow
              label="Available immediately?"
              value={cond.availableImmediately}
              onChange={(b) => setCond({ ...cond, availableImmediately: b })}
            />
      ...
      {step === "media" && selected && (
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Camera className="h-5 w-5" /> Photos & documents
            </CardTitle>
            <p className="text-xs text-muted-foreground">
              Clear photos and valid documents speed up {selected.name}'s approval.
            </p>
          </CardHeader>
          <CardContent className="space-y-5">
            ...
            <div className="space-y-3">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <Fld
                  label="Front photo (cover) *"
                  value={m.coverPhoto}
                  onChange={(x) => setM({ ...m, coverPhoto: x })}
                  placeholder="https://…"
                />
                <Fld
                  label="Rear photo"
                  value={m.rearPhoto}
                  onChange={(x) => setM({ ...m, rearPhoto: x })}
                  placeholder="https://…"
                />
              </div>
              <div>
                ...
                <Textarea
                  rows={2}
                  value={m.sidePhotos}
                  onChange={(e) => setM({ ...m, sidePhotos: e.target.value })}
                  placeholder={"https://left-side.jpg\nhttps://right-side.jpg"}
                />
              </div>
              <div>
                ...
                <Textarea
                  rows={2}
                  value={m.interiorPhotos}
                  onChange={(e) => setM({ ...m, interiorPhotos: e.target.value })}
                  placeholder={"https://interior-front.jpg\nhttps://interior-rear.jpg"}
                />
              </div>
              <div>
                ...
                <Textarea
                  rows={2}
                  value={m.additionalImages}
                  onChange={(e) => setM({ ...m, additionalImages: e.target.value })}
                  placeholder="Any other angles, dashboard, boot…"
                />
                <p className="mt-1 text-xs text-muted-foreground">
                  Max 20 photos total across all sections.
                </p>
              </div>
              <Fld
                label="Video walk-around (optional)"
                value={m.videoUrl}
                onChange={(x) => setM({ ...m, videoUrl: x })}
                placeholder="https://…"
              />
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <Fld
                  label="Logbook / proof of ownership *"
                  value={m.docLogbook}
                  onChange={(x) => setM({ ...m, docLogbook: x })}
                  placeholder="https://…"
                />
                <Fld
                  label="Insurance certificate *"
                  value={m.docInsurance}
                  onChange={(x) => setM({ ...m, docInsurance: x })}
                  placeholder="https://…"
                />
                <Fld
                  label="National ID *"
                  value={m.docNationalId}
                  onChange={(x) => setM({ ...m, docNationalId: x })}
                  placeholder="https://…"
                />
                <Fld
                  label="Inspection certificate (optional)"
                  value={m.docInspection}
                  onChange={(x) => setM({ ...m, docInspection: x })}
                  placeholder="https://…"
                />
              </div>
            </div>
      ...
      {step === "prefs" && selected && (
        <Card>
          <CardHeader>
            ...
            <p className="text-xs text-muted-foreground">
              Tell {selected.name} how you'd like your vehicle managed.
            </p>
          </CardHeader>
          <CardContent className="space-y-5">
            <ApplyStepper step={step} />
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                {[
                  ...
                  <button
                    key={o.id}
                    type="button"
                    onClick={() => setPrefs({ ...prefs, serviceType: o.id as any })}
            ...
            <div>
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                {[
                  ...
                  <button
                    key={o.id}
                    type="button"
                    onClick={() => setPrefs({ ...prefs, availability: o.id as any })}
              ...
              {prefs.availability === "specific_dates" && (
                <div className="mt-3">
                  ...
                  <Textarea
                    rows={2}
                    value={prefs.availabilityDates}
                    onChange={(e) => setPrefs({ ...prefs, availabilityDates: e.target.value })}
                    placeholder="e.g. Available 1 Dec – 31 Jan, then weekends"
                  />
                </div>
              )}
            </div>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <Fld
                ...
                type="number"
                value={String(prefs.minRentalDays)}
                onChange={(x) => setPrefs({ ...prefs, minRentalDays: Number(x) })}
              />
              <Fld
                label="Suggested daily rate (KES, optional)"
                type="number"
                value={String(prefs.proposedDailyRateKes)}
                onChange={(x) => setPrefs({ ...prefs, proposedDailyRateKes: Number(x) })}
              />
            </div>
      ...
      {step === "review" && selected && (
        <Card>
          <CardHeader>
            ...
            <p className="text-xs text-muted-foreground">
              Take a moment to confirm everything before submitting to {selected.name}.
            </p>
          </CardHeader>
          <CardContent className="space-y-4">
            <ApplyStepper step={step} />
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <ReviewBlock title="Vehicle location">
                <div>
                  {loc.town || "—"}
                  {loc.countyCode ? ` · County ${loc.countyCode}` : ""}
                </div>
                {loc.pickupLocation && (
                  <div className="text-xs text-muted-foreground">Pickup: {loc.pickupLocation}</div>
                )}
              </ReviewBlock>
              <ReviewBlock title="Applicant">
                ...
                <div className="text-xs text-muted-foreground">
                  Payment: {p.preferredPaymentMethod}
                </div>
              </ReviewBlock>
              ...
              <ReviewBlock title="Condition">
                <div className="text-xs text-muted-foreground capitalize">
                  ...
                  {cond.insured ? " · insured" : ""}
                  {cond.roadworthy ? " · roadworthy" : ""}
              ...
              <ReviewBlock title="Photos & documents">
                ...
                <div className="text-xs text-muted-foreground">
                  {[
                    m.docLogbook && "logbook",
                    m.docInsurance && "insurance",
                    m.docNationalId && "national ID",
                    m.docInspection && "inspection",
                  ]
                    .filter(Boolean)
                    .join(" · ") || "No documents"}
                </div>
              ...
              <ReviewBlock title="Partnership preferences">
                <div className="text-xs text-muted-foreground capitalize">
                  {prefs.serviceType.replace("_", "-")} · {prefs.availability.replace("_", " ")} ·
                  min {prefs.minRentalDays} day{prefs.minRentalDays === 1 ? "" : "s"}
                </div>
                {prefs.proposedDailyRateKes > 0 && (
                  <div className="text-xs text-muted-foreground">
                    Suggested rate: KES {prefs.proposedDailyRateKes.toLocaleString()}/day
                  </div>
                )}
              </ReviewBlock>
            </div>
            <div className="rounded-md border bg-muted/40 p-3 text-xs text-muted-foreground">
              On submission, your vehicle status becomes <strong>Pending company approval</strong>.
              Your vehicle will not appear publicly until <strong>{selected.name}</strong> approves
              the partnership. They will then manage bookings, customer communication and pricing on
              your behalf.
            </div>
            <div className="flex justify-between">
              <Button variant="ghost" onClick={() => back(step)}>
                <ArrowLeft className="mr-1 h-4 w-4" /> Back
              </Button>
              <Button
                onClick={() => submitMut.mutate()}
                disabled={submitMut.isPending || !v.make || !v.model || !p.legalName || !p.phone}
              >
                {submitMut.isPending ? (
                  "Submitting…"
                ) : (
                  <>
                    <Handshake className="mr-1 h-4 w-4" /> Submit Partnership Application
                  </>
                )}
              </Button>
            </div>
        ...
        <Card>
          <CardContent className="space-y-4 py-10 text-center">
            ...
            <p className="mx-auto max-w-xl text-sm text-muted-foreground">
              ...
              <strong className="text-foreground">{selected.name}</strong>. Their fleet team will
              review your vehicle and contact you if additional information or an inspection is
              required. Your vehicle status is now <strong>Pending company approval</strong>.
            </p>
            <p className="text-xs text-muted-foreground">
              Track progress from My Fleet Partnerships.
            </p>
...
function CompanyCard({ p, onPartner }: { p: any; onPartner: () => void }) {
  ...
  return (
    <div className="flex h-full flex-col rounded-lg border bg-card p-4 transition hover:border-primary hover:shadow-sm">
      <div className="flex items-start gap-3">
        {p.logo_url ? (
          <img src={p.logo_url} alt="" className="h-12 w-12 rounded object-cover" />
        ) : (
          <div className="h-12 w-12 rounded bg-muted" />
        )}
        <div className="min-w-0 flex-1">
          ...
          <div className="text-xs text-muted-foreground">
            {p.town ?? "Kenya"}
            {p.county_code ? ` · County ${p.county_code}` : ""}
            {p.rating_avg
              ? ` · ★ ${Number(p.rating_avg).toFixed(1)}${p.rating_count ? ` (${p.rating_count})` : ""}`
              : ""}
          </div>
    ...
    <div className="flex items-center justify-between rounded-md border bg-muted/40 p-3">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <div className="text-xs text-muted-foreground">
            {p.town ?? "Kenya"} · {p.private_owner_commission_pct}% commission to company
          </div>
        </div>
      </div>
...
// ============================================================================
function PartnershipsDashboard({
  owner,
  onNewPartnership,
}: {
  owner: any;
    ...
    <div className="space-y-6">
      <header className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="max-w-2xl text-sm text-muted-foreground">
            Track your vehicle partnerships with HostPulse rental companies. You retain ownership —
            your partner companies handle bookings, customers and day-to-day operations.
          </p>
        </div>
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-xs text-muted-foreground">
            Net payouts after each rental company's commission. Includes confirmed and completed
            bookings.
          </p>
        </CardHeader>
        <CardContent>
          {earnings.isLoading ? (
            ...
            <EmptyState
              title="No approved partnerships yet"
              description="Once a company approves your vehicle, earnings will appear here."
            />
          ) : (
            <div className="space-y-4">
              <div className="grid gap-3 sm:grid-cols-4">
                ...
                <Stat
                  ...
                  value={`KES ${earnings.data.totals.commission.toLocaleString()}`}
                  tone="muted"
                />
                <Stat
                  label="Your net payout"
                  value={`KES ${earnings.data.totals.net.toLocaleString()}`}
                  tone="primary"
                />
                  ...
                  {earnings.data.byVehicle.map((row: any) => (
                    ...
                    >
                      <div>
                        ...
                        <div className="text-xs text-muted-foreground">
                          Managed by {row.vehicle?.mobility_providers?.name ?? "—"} · {row.count}{" "}
                          booking{row.count === 1 ? "" : "s"}
                        </div>
                      </div>
                      <div className="text-right">
                        <div className="font-medium">KES {row.net.toLocaleString()}</div>
                        <div className="text-xs text-muted-foreground">
                          of {row.gross.toLocaleString()} gross
                        </div>
                      </div>
                    </div>
                  ))}
      ...
      <OwnerProfilePanel
        owner={owner}
        onSaved={() => qc.invalidateQueries({ queryKey: ["mob-owner"] })}
      />
    </div>
  );
}
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <p className="text-xs text-muted-foreground">
          Track applications under review, approved partnerships, and past decisions.
        </p>
      </CardHeader>
      <CardContent className="space-y-4">
        {subs.isLoading ? (
          ...
          <EmptyState
            title="No partnership applications yet"
            description="Choose a rental company to submit your first vehicle."
          />
        ) : (
          <>
            {pending.length > 0 && (
              <ApplicationGroup
                title="Vehicles under review"
            ...
            {approved.length > 0 && (
              <ApplicationGroup
                title="Vehicles under management"
                icon={<BadgeCheck className="h-4 w-4 text-emerald-600" />}
                items={approved}
              />
            )}
            {other.length > 0 && <ApplicationGroup title="Past applications" items={other} />}
          </>
        )}
      </CardContent>
    </Card>
        ...
        return (
          ...
          >
            <div className="min-w-0">
              ...
              <div className="text-xs text-muted-foreground">
                Managed by {s.mobility_providers?.name ?? "…"} · applied{" "}
                {new Date(s.created_at).toLocaleDateString()}
              </div>
              {s.decision_reason && (
                <div className="mt-1 text-xs text-muted-foreground">
                  Company note: {s.decision_reason}
                </div>
              )}
            </div>
            <div className="flex items-center gap-2">
              <Badge className={STATUS_BADGE[s.status] ?? ""}>
                {STATUS_LABEL[s.status] ?? s.status}
              </Badge>
              {s.status === "pending" && onWithdraw && (
                <Button size="sm" variant="ghost" onClick={() => onWithdraw(s.id)}>
                  Withdraw
                </Button>
              )}
...
function OwnerProfilePanel({ owner, onSaved }: { owner: any; onSaved: () => void }) {
  ...
  const [d, setD] = useState({
    ...
    preferredPaymentMethod: (owner.preferred_payment_method ?? "mpesa") as
      | "mpesa"
      | "bank"
      | "both",
    ...
    return (
      <Card>
        ...
        <CardContent className="grid gap-1 text-sm text-muted-foreground sm:grid-cols-2">
          <div>
            <span className="font-medium text-foreground">{owner.legal_name}</span>
          </div>
          <div>
          ...
          <div>
            {owner.town ?? "—"}
            {owner.county_code ? ` · County ${owner.county_code}` : ""}
          </div>
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          <User className="h-5 w-5" /> Edit my profile
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="grid gap-3 sm:grid-cols-2">
          <Fld
            label="Full name *"
            value={d.legalName}
            onChange={(v) => setD({ ...d, legalName: v })}
          />
          <Fld
            label="National ID / Passport"
            value={d.idNumber}
            onChange={(v) => setD({ ...d, idNumber: v })}
          />
          ...
          <Fld
            label="County"
            value={d.countyCode}
            onChange={(v) => setD({ ...d, countyCode: v })}
          />
          ...
          <Fld
            ...
            onChange={(v) => setD({ ...d, emergencyContact: v })}
          />
          <div>
            <Label>Preferred payment method</Label>
            <select
...
function Stat({
  ...
  return (
    <div
      className={`rounded-md border p-3 ${tone === "primary" ? "border-primary/40 bg-primary/5" : ""}`}
    >
      <div className="text-xs text-muted-foreground">{label}</div>
      <div
        className={`mt-1 text-lg font-semibold ${tone === "muted" ? "text-muted-foreground" : ""}`}
      >
        {value}
      </div>
    </div>
...
function PayoutsCard() {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          <Coins className="h-5 w-5" /> Upcoming payments
        </CardTitle>
        <p className="text-xs text-muted-foreground">
          Request payouts against your net earnings. HostPulse processes M-Pesa or bank transfers.
        </p>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="rounded-md border bg-muted/40 p-3 text-sm">
          Available for payout:{" "}
          <span className="font-semibold">KES {Number(available).toLocaleString()}</span>
        </div>
        <div className="grid gap-3 sm:grid-cols-4">
          <div>
          ...
          <div className="sm:col-span-2">
            ...
            <Input
              ...
              placeholder={method === "mpesa" ? "07XXXXXXXX" : "Bank / account number"}
            />
          </div>
              ...
              >
                <div>
                  <div className="font-medium">
                    ...
                    <span className="text-xs uppercase">{r.method}</span>
                  </div>
                  <div className="text-xs text-muted-foreground">
