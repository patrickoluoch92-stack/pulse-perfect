    ...
    mutationFn: () =>
      requestFn({
        data: {
          ...
          method,
          destination:
            method === "mpesa" ? { phone } : { account_number: bankAcct, bank_name: bankName },
    ...
    <div className="mx-auto max-w-6xl space-y-6 p-6">
      <header className="flex items-center justify-between">
        <div>
          <h1 className="flex items-center gap-2 text-2xl font-semibold">
            <Wallet className="h-6 w-6" /> Wallet & Payouts
          </h1>
          <p className="text-sm text-muted-foreground">
            Earnings from confirmed bookings settle to your wallet after guest check-out.
          </p>
        </div>
          ...
          <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent>
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">
                    Available: {fmt(w?.available_balance, currency)}
                  </p>
                </div>
                <div>
                  <Label>Method</Label>
              ...
              <DialogFooter className="flex-col gap-2 sm:flex-row">
                ...
                <Button onClick={() => request.mutate()} disabled={request.isPending || !amount}>
                  Submit request
                </Button>
              </DialogFooter>
            </DialogContent>
        ...
        <Card>
          ...
          <CardContent>
            <p className="text-2xl font-semibold text-primary">
              {fmt(w?.available_balance, currency)}
            </p>
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent>
            <p className="text-2xl font-semibold">{fmt(w?.pending_balance, currency)}</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm text-muted-foreground">Lifetime earned</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-2xl font-semibold">{fmt(w?.lifetime_earned, currency)}</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
          ...
          <CardContent>
            <p className="text-2xl font-semibold">{fmt(w?.lifetime_paid_out, currency)}</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        ...
        <CardContent className="p-0">
          {payouts.data?.rows?.length === 0 && <EmptyState title="No payouts yet" />}
      ...
      <Card>
        ...
        <CardContent className="p-0">
          {ledger.data?.rows?.length === 0 && <EmptyState title="No activity yet" />}
          <ul className="divide-y">
