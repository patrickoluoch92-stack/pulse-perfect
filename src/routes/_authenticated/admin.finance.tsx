...
function FinanceAdmin() {
  ...
  const [status, setStatus] = useState<
    "requested" | "approved" | "processing" | "paid" | "failed" | "cancelled" | "all"
  >("requested");
  const [payDialog, setPayDialog] = useState<{ id: string; open: boolean }>({
    id: "",
    open: false,
  });
  const [reference, setReference] = useState("");
  const [failDialog, setFailDialog] = useState<{ id: string; open: boolean }>({
    id: "",
    open: false,
  });
    ...
    <div className="mx-auto max-w-7xl space-y-6 p-6">
      <header className="flex items-center justify-between">
        <div>
          <h1 className="flex items-center gap-2 text-2xl font-semibold">
            <Landmark className="h-6 w-6" /> Finance admin
          </h1>
          <p className="text-sm text-muted-foreground">
            Platform revenue, wallet balances, and payout queue.
          </p>
        </div>
        <Button
          variant="outline"
        ...
        <Card>
          ...
          <CardContent>
            <p className="text-2xl font-semibold text-primary">
              {fmt(o?.commissionsAllTime.commission ?? 0)}
            </p>
            <p className="mt-1 text-xs text-muted-foreground">
              Gross booked: {fmt(o?.commissionsAllTime.gross ?? 0)}
            </p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm text-muted-foreground">Commission (30d)</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-2xl font-semibold">{fmt(o?.commissions30d.commission ?? 0)}</p>
            <p className="mt-1 text-xs text-muted-foreground">
              Gross: {fmt(o?.commissions30d.gross ?? 0)}
            </p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm text-muted-foreground">Taxes collected</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-2xl font-semibold">
              {fmt(
                (o?.commissionsAllTime.vat ?? 0) +
                  (o?.commissionsAllTime.levy ?? 0) +
                  (o?.commissionsAllTime.service_fee ?? 0),
              )}
            </p>
            <p className="mt-1 text-xs text-muted-foreground">VAT + Levy + Service fee</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm text-muted-foreground">Payouts</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-2xl font-semibold">{o?.payouts.requestedCount ?? 0} requested</p>
            <p className="mt-1 text-xs text-muted-foreground">
              Pending: {fmt(o?.payouts.pendingAmount ?? 0)} · Paid 30d:{" "}
              {fmt(o?.payouts.paid30d ?? 0)}
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm text-muted-foreground">
              Wallet available (all owners)
            </CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-xl font-semibold">{fmt(o?.wallets.availableTotal ?? 0)}</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm text-muted-foreground">Wallet pending</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-xl font-semibold">{fmt(o?.wallets.pendingTotal ?? 0)}</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm text-muted-foreground">
              Lifetime earned by owners
            </CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-xl font-semibold">{fmt(o?.wallets.lifetimeEarned ?? 0)}</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm text-muted-foreground">Lifetime paid out</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-xl font-semibold">{fmt(o?.wallets.lifetimePaidOut ?? 0)}</p>
          </CardContent>
        </Card>
      </div>
                    ...
                    {(p.status === "approved" || p.status === "processing") && (
                      <>
                        <Button
                          size="sm"
                          variant="default"
                          onClick={() => setPayDialog({ id: p.id, open: true })}
                        >
                        ...
                        <Button
                          size="sm"
                          variant="destructive"
                          onClick={() => setFailDialog({ id: p.id, open: true })}
                        >
        ...
        <DialogContent>
          ...
          <div>
            ...
            <Input
              ...
              placeholder="e.g. QHT7X9AB2Z"
            />
          </div>
          <DialogFooter>
            <Button onClick={() => markPaid.mutate()} disabled={!reference || markPaid.isPending}>
              Confirm
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog
        ...
        onOpenChange={(o) => setFailDialog({ ...failDialog, open: o })}
      >
        <DialogContent>
          <DialogHeader>
          ...
          <div>
            <Label>Reason (refunds funds to owner)</Label>
            <Input
              value={reason}
