...
export async function pickCommissionRule(admin: Admin, opts: {
  orgId: string; propertyId: string; category: string | null; countyCode: string | null;
}) {
...
export async function writeLedger(admin: Admin, opts: {
  walletId: string; orgId: string; entryType: "credit" | "debit";
  category: string; amount: number;
  targetBucket: "available" | "pending"; movePendingToAvailable?: boolean;
  referenceType?: string; referenceId?: string; description?: string; createdBy?: string;
}): Promise<string> {
  const { data: wallet, error: wErr } = await admin
    .from("owner_wallets").select("*").eq("id", opts.walletId).single();
  ...
  const { data: entry, error: lErr } = await admin
    .from("wallet_ledger")
    .insert({
      wallet_id: opts.walletId, org_id: opts.orgId,
      entry_type: opts.entryType, category: opts.category,
      amount: amt,
      available_after: available, pending_after: pending,
      reference_type: opts.referenceType ?? null, reference_id: opts.referenceId ?? null,
      description: opts.description ?? null, created_by: opts.createdBy ?? null,
    })
    .select("id")
    .single();
  ...
  const { error: uErr } = await admin
    ...
    .update({
      ...
      lifetime_earned: lifetimeEarned, lifetime_paid_out: lifetimePaidOut,
    })
    .eq("id", opts.walletId);
...
export async function accrueForBooking(bookingId: string, createdBy?: string) {
  ...
  const { data: prop, error: pErr } = await admin
    ...
    .eq("id", b.property_id).single();
  if (pErr || !prop) throw new Error("Property not found");
  const p = prop as any;

  const { data: existing } = await admin
    .from("booking_commissions").select("*").eq("booking_id", b.id).maybeSingle();
  ...
  const rule = await pickCommissionRule(admin, {
    orgId: p.org_id, propertyId: p.id, category: p.category, countyCode: p.county_code,
  });
  ...
  const ledgerId = await writeLedger(admin, {
    walletId: (wallet as any).id, orgId: p.org_id,
    entryType: "credit", category: "booking_earnings",
    amount: netOwner, targetBucket: "pending",
    referenceType: "booking", referenceId: b.id,
    description: `Booking ${b.id.slice(0,8)} pending settlement`,
    createdBy,
  });

  const { data: comm, error: cErr } = await admin
    .from("booking_commissions").insert({
      booking_id: b.id, org_id: p.org_id, property_id: p.id,
      gross_amount: gross, commission_rate: rate, commission_amount: commission,
      vat_amount: vat, levy_amount: levy, service_fee_amount: svc,
      net_owner_amount: netOwner, currency: b.currency ?? "KES",
      rule_id: (rule as any)?.id ?? null,
      status: "pending", credited_ledger_id: ledgerId,
    }).select("*").single();
  if (cErr) throw new Error(cErr.message);
...
export async function settleForBooking(bookingId: string, createdBy?: string) {
  ...
  const { data: comm, error } = await admin
    .from("booking_commissions").select("*").eq("booking_id", bookingId).maybeSingle();
  if (error) throw new Error(error.message);
  ...
  const ledgerId = await writeLedger(admin, {
    ...
    orgId: c.org_id,
    entryType: "credit", category: "booking_earnings",
    amount: Number(c.net_owner_amount), targetBucket: "available",
    movePendingToAvailable: true,
    referenceType: "booking", referenceId: c.booking_id,
    description: `Booking ${c.booking_id.slice(0,8)} settled`,
    createdBy,
  });
  await admin.from("booking_commissions")
...
export async function reverseForBooking(bookingId: string, createdBy?: string) {
  ...
  const ledgerId = await writeLedger(admin, {
    walletId: (await resolveWallet(admin, c.org_id) as any).id,
    orgId: c.org_id,
    entryType: "debit", category: "refund",
    amount: Number(c.net_owner_amount), targetBucket: bucket as "available" | "pending",
    referenceType: "booking", referenceId: c.booking_id,
