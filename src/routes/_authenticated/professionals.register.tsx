    ...
    <div className="mx-auto max-w-3xl space-y-6 p-4 md:p-6">
        ...
        <CardContent className="space-y-4">
          {step === 0 && (
            <>
              ...
              <Field label="Tagline">
                <Input maxLength={160} value={form.tagline ?? ""} onChange={(e) => set("tagline", e.target.value)} />
              </Field>
          ...
          {step === 2 && (
            <>
              <div className="grid gap-4 md:grid-cols-3">
                <Field label="County code"><Input value={form.county_code ?? ""} onChange={(e) => set("county_code", e.target.value)} /></Field>
                <Field label="Town"><Input value={form.town ?? ""} onChange={(e) => set("town", e.target.value)} /></Field>
                <Field label="Area / estate"><Input value={form.area ?? ""} onChange={(e) => set("area", e.target.value)} /></Field>
              </div>
              <div className="space-y-2 rounded border p-3">
                <label className="flex items-center gap-2 text-sm">
                  <Checkbox checked={!!form.nationwide} onCheckedChange={(v) => set("nationwide", !!v)} /> Serves nationwide
                </label>
                <label className="flex items-center gap-2 text-sm">
                  <Checkbox checked={!!form.travels_to_clients} onCheckedChange={(v) => set("travels_to_clients", !!v)} /> Travels to clients
                </label>
                <label className="flex items-center gap-2 text-sm">
                  <Checkbox checked={!!form.online_services} onCheckedChange={(v) => set("online_services", !!v)} /> Offers online services
                </label>
                <Field label="Max travel distance (km)">
                  <Input type="number" min={0} value={form.max_travel_km ?? ""} onChange={(e) => set("max_travel_km", e.target.value)} />
                </Field>
              </div>
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <Field label="Website"><Input type="url" value={form.website ?? ""} onChange={(e) => set("website", e.target.value)} /></Field>
              </div>
            </>
          )}
            ...
            <>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <Field label="Starting price (KES)">
                  <Input type="number" min={0} value={form.starting_price ?? ""} onChange={(e) => set("starting_price", e.target.value)} />
                </Field>
          ...
          {step === 4 && (
            <div className="space-y-3 text-sm">
              ...
              <Row k="Pricing" v={form.pricing_model ? `${form.pricing_model} · KES ${form.starting_price ?? "—"}` : "—"} />
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => save(false)} disabled={saving || !form.business_name}>
            Save draft
          </Button>
          {step < STEPS.length - 1 ? (
