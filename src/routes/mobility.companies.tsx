...
export const Route = createFileRoute("/mobility/companies")({
  ...
  head: () => ({ meta: [
    ...
    { name: "description", content: "Browse verified car hire, chauffeur and safari transport companies across Kenya." },
  ] }),
});
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 p-6">
      <header className="space-y-2">
        <h1 className="flex items-center gap-2 text-3xl font-semibold"><Car className="h-7 w-7" /> Verified car hire companies</h1>
        <p className="text-muted-foreground">Trusted fleets across Kenya — self-drive, chauffeur, safari and luxury transport.</p>
      </header>
      <div className="relative max-w-md">
        <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
        <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search companies…" className="pl-9" />
      </div>
      {query.isLoading ? <LoadingState label="Loading companies…" /> : (query.data?.providers ?? []).length === 0 ? (
          ...
          {query.data!.providers.map((p: any) => (
            <Link key={p.id} to="/mobility/company/$slug" params={{ slug: p.slug }}>
              <Card className="h-full overflow-hidden transition hover:shadow-md">
                {p.cover_image_url && <img src={p.cover_image_url} alt="" className="h-32 w-full object-cover" />}
                <CardContent className="p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="text-xs text-muted-foreground">{p.town ?? "Kenya"}{p.rating_avg ? ` · ★ ${Number(p.rating_avg).toFixed(1)}` : ""}</div>
                    </div>
                  </div>
                  {p.bio && <p className="mt-2 line-clamp-2 text-sm text-muted-foreground">{p.bio}</p>}
                  {(p.service_categories ?? []).length > 0 && (
                    <div className="mt-2 flex flex-wrap gap-1">
                      {(p.service_categories as string[]).slice(0, 3).map((c) => <Badge key={c} variant="outline" className="text-xs">{c.replace(/_/g, " ")}</Badge>)}
                    </div>
                  )}
                </CardContent>
              </Card>
