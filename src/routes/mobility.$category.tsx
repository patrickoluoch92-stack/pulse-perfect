  ...
  head: ({ params }) => {
    ...
    return {
      meta: [
        ...
        {
          name: "description",
          content: `Book ${label.toLowerCase()} across Kenya on HostPulse. Compare vehicles, pickup locations, and daily rates.`,
        },
        { property: "og:title", content: `${label} in Kenya` },
        {
          property: "og:description",
          content: `Compare ${label.toLowerCase()} across Kenya on HostPulse.`,
        },
      ],
    };
  },
...
function CategoryPage() {
  ...
  const [filters, setFilters] = useState<{
    vehicleType?: MobilityVehicleType;
    make?: string;
    minSeats?: number;
    priceMaxKes?: number;
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      fetchList({
        data: {
          category: category as MobilityCategory,
          limit: 30,
          ...filters,
    ...
    <div className="min-h-dvh bg-background">
      <section className="mx-auto max-w-6xl px-6 py-10">
        ...
        <p className="mt-2 max-w-2xl text-muted-foreground">
          Browse available {label.toLowerCase()} across Kenya. Book directly through HostPulse.
        </p>
        ...
        <div className="mt-6 grid gap-3 rounded-lg border bg-card p-4 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-7">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setFilters({ ...filters, vehicleType: (e.target.value || undefined) as any })
              }
            >
              <option value="">Any</option>
              {MOBILITY_VEHICLE_TYPES.map((t) => (
                <option key={t} value={t}>
                  {MOBILITY_VEHICLE_TYPE_LABELS[t]}
                </option>
              ))}
            </select>
          </div>
          ...
          <div>
            ...
            <Input
              ...
              onChange={(e) =>
                setFilters({
                  ...filters,
                  priceMaxKes: e.target.value ? Number(e.target.value) : undefined,
                })
              }
            />
          </div>
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setFilters({ ...filters, transmission: (e.target.value || undefined) as any })
              }
            >
              <option value="">Any</option>
