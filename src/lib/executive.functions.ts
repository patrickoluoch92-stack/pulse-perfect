...
export const getExecutiveOverview = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const [
      props, bookings, commissions, subs, wallets, payouts, users, orgs,
      discovered, claims, appErrors, reviews, coupons, tickets,
    ] = await Promise.all([
      s.from("marketplace_properties").select("id, status, is_published, is_featured, verification_status, org_id, created_at"),
      s.from("marketplace_bookings").select("id, status, total_amount, check_in, check_out, created_at, property_id"),
      s.from("booking_commissions").select("id, gross_amount, commission_amount, net_amount, status, created_at"),
      s.from("subscriptions").select("id, plan, status, current_period_end, price_amount, created_at"),
      s.from("owner_wallets").select("org_id, available_balance, pending_balance, lifetime_earnings"),
      ...
      s.from("app_errors").select("id, severity, created_at").order("created_at", { ascending: false }).limit(200),
    ...
    const errors = [props, bookings, commissions, subs, wallets, payouts, users, orgs, discovered, claims, appErrors, reviews, coupons, tickets]
    ...
    const sinceRev = (from: Date) => bList
      .filter(b => ["confirmed","completed"].includes(b.status) && new Date(b.created_at) >= from)
      .reduce((s, b) => s + Number(b.total_amount ?? 0), 0);

    const commissionSince = (from: Date) => cList
    ...
    const upcomingCheckins = bList.filter(b => new Date(b.check_in) >= today && new Date(b.check_in) < addDays(today, 8) && b.status !== "cancelled").length;
    const upcomingCheckouts = bList.filter(b => new Date(b.check_out) >= today && new Date(b.check_out) < addDays(today, 8) && b.status !== "cancelled").length;
    ...
    const errCritical = errRecent.filter(e => e.severity === "critical" || e.severity === "error").length;

    const reviewsList = (reviews.data ?? []) as any[];
    const avgRating = reviewsList.length
      ? Math.round((reviewsList.reduce((s, r) => s + Number(r.rating ?? 0), 0) / reviewsList.length) * 10) / 10
      : null;

    const walletOutstanding = wList.reduce((s, w) => s + Number(w.available_balance ?? 0) + Number(w.pending_balance ?? 0), 0);
    const pendingPayouts = payList.filter(p => p.status === "requested" || p.status === "processing").length;
    const pendingPayoutAmount = payList.filter(p => p.status === "requested" || p.status === "processing")
    ...
    const activity = [
      ...bList.slice(-15).map((b) => ({ type: "booking", at: b.created_at, message: `Booking ${b.status} · KES ${Number(b.total_amount ?? 0).toLocaleString()}` })),
      ...((users.data ?? []) as any[]).slice(-10).map((u) => ({ type: "signup", at: u.created_at, message: "New user registered" })),
      ...errRecent.slice(0, 10).map((e) => ({ type: "error", at: e.created_at, message: `System ${e.severity ?? "info"}` })),
    ...
    return {
      ...
      discovery: {
        pending: ((discovered.data ?? []) as any[]).filter(d => d.status === "pending_review").length,
        published: ((discovered.data ?? []) as any[]).filter(d => d.status === "published").length,
        claims: ((claims.data ?? []) as any[]).filter(c => c.status === "pending").length,
      },
      users: {
        total: (users.data ?? []).length,
        newToday: ((users.data ?? []) as any[]).filter((u) => new Date(u.created_at) >= today).length,
        newMonth: ((users.data ?? []) as any[]).filter((u) => new Date(u.created_at) >= monthStart).length,
      },
      organizations: {
        total: (orgs.data ?? []).length,
        newMonth: ((orgs.data ?? []) as any[]).filter((o) => new Date(o.created_at) >= monthStart).length,
      },
      bookings: {
        today: bookingsToday,
        month: bookingsMonth,
      ...
      finance: {
        ...
        pendingPayouts,
        pendingPayoutAmount,
        payoutsProcessedMonth: payList.filter(p => p.status === "completed" && p.processed_at && new Date(p.processed_at) >= monthStart)
          .reduce((s, p) => s + Number(p.amount ?? 0), 0),
      },
      support: {
        openTickets: ((tickets.data ?? []) as any[]).filter(t => t.status !== "resolved").length,
        urgentTickets: ((tickets.data ?? []) as any[]).filter(t => t.priority === "urgent" && t.status !== "resolved").length,
        avgRating,
      ...
      health: {
        db: dbHealth, api: apiHealth, payment: paymentHealth, ai: aiHealth,
        errorsWeek: errWeek, errorsCritical: errCritical,
      },
      activity,
