...
const emptyPlan = {
  ...
  price_yearly_kes: 0,
  trial_days: 0,
  property_limit: null as number | null,
  photo_limit_per_property: null as number | null,
  storage_mb: null as number | null,
  ai_calls_per_month: null as number | null,
  team_member_limit: null as number | null,
  has_api_access: false,
  has_priority_support: false,
  has_dynamic_pricing: false,
  has_channel_manager: false,
  has_promotional_boost: false,
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 p-6">
          ...
          <Card key={p.id}>
            <CardContent className="flex items-center justify-between py-4">
              ...
              <div className="flex items-center gap-2">
                <Switch
                  checked={p.active}
                  onCheckedChange={(v) => toggleM.mutate({ id: p.id, active: v })}
                />
      ...
      {editing && (
        <Card>
          <CardHeader>
            <CardTitle>{editing.id ? "Edit plan" : "New plan"}</CardTitle>
          </CardHeader>
          <CardContent className="grid gap-4 md:grid-cols-2">
            {[
              ...
              ["name", "Name"],
              ["tagline", "Tagline"],
              ["price_monthly_kes", "Monthly (KES)"],
              ["price_yearly_kes", "Yearly (KES)"],
              ["trial_days", "Trial days"],
              ["property_limit", "Property limit"],
              ["photo_limit_per_property", "Photos / property"],
              ["storage_mb", "Storage MB"],
              ["ai_calls_per_month", "AI calls / month"],
              ...
              <div key={k} className="space-y-1">
                ...
                <Input
                  value={editing[k] ?? ""}
                  type={
                    typeof emptyPlan[k as keyof typeof emptyPlan] === "number" ? "number" : "text"
                  }
                  onChange={(e) => {
                    const raw = e.target.value;
                    const numeric =
                      typeof emptyPlan[k as keyof typeof emptyPlan] === "number" ||
                      emptyPlan[k as keyof typeof emptyPlan] === null;
                    setEditing({
                      ...editing,
                      [k]: numeric ? (raw === "" ? null : Number(raw)) : raw,
                    });
                  }}
                />
              </div>
            ...
            {[
              ...
              >
                ...
                <Switch
                  checked={!!editing[k]}
                  onCheckedChange={(v) => setEditing({ ...editing, [k]: v })}
                />
              </label>
            ))}
