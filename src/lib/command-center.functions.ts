...
export const getOwnerCommandCenter = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const publishedCount = (props ?? []).filter(
    ...
    const bookingsQ = propertyIds.length
      ? supabase
          ...
          .select(
            "id, status, total_amount, check_in, check_out, guest_name, guest_email, created_at, property_id",
          )
    ...
    const [bookingsRes, wallet, subQ, unitsQ, ticketsQ, hkQ, reviewsQ] = await Promise.all([
      ...
      supabase
        ...
        .select("rating, comment, created_at, property_id")
        .in(
          "property_id",
          propertyIds.length ? propertyIds : ["00000000-0000-0000-0000-000000000000"],
        )
        .order("created_at", { ascending: false })
        .limit(20),
    ]);
    ...
    const checkInsToday = bookings.filter(
      (b) => b.check_in === todayISO && ["confirmed", "pending"].includes(b.status),
    );
    const checkOutsToday = bookings.filter(
      (b) => b.check_out === todayISO && ["confirmed", "completed"].includes(b.status),
    );
    ...
    const avgRating = reviews.length
      ? Math.round((reviews.reduce((s, r) => s + Number(r.rating ?? 0), 0) / reviews.length) * 10) /
        10
      : null;
    ...
    if ((ticketsQ.data ?? []).some((t: any) => t.priority === "urgent")) {
      alerts.push({
        level: "error",
        message: "Urgent maintenance tickets need attention",
        href: "/maintenance",
      });
    }
    ...
    if (publishedCount === 0 && propertyCount > 0) {
      alerts.push({
        level: "warn",
        message: "You have unpublished properties. Publish them to start receiving bookings.",
        href: "/listings",
      });
    }
    ...
    if (!s || s?.status !== "active")
      alerts.push({
        level: "info",
        message: "Upgrade your plan to unlock premium features",
        href: "/pricing",
      });
    ...
    return {
      ...
      subscription: s ?? null,
      maintenance: {
        open: (ticketsQ.data ?? []).length,
        urgent: (ticketsQ.data ?? []).filter((t: any) => t.priority === "urgent").length,
      },
