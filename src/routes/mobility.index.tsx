  ...
  head: () => ({
    meta: [
      { title: "Car Hire & Mobility in Kenya | HostPulse" },
      {
        name: "description",
        content:
          "Self-drive rentals, chauffeur services, airport transfers, safari 4x4s, tour vans, luxury cars and more across Kenya. Book on HostPulse.",
      },
      { property: "og:title", content: "Car Hire & Mobility in Kenya" },
      {
        property: "og:description",
        content:
          "Rent cars, safari vehicles, buses, motorcycles, and boats across Kenya on HostPulse.",
      },
      ...
      <section className="border-b bg-gradient-to-b from-primary/5 to-transparent">
        <div className="mx-auto max-w-6xl px-6 py-16">
          ...
          <p className="mt-3 max-w-2xl text-lg text-muted-foreground">
            Book self-drive cars, chauffeur services, safari 4x4s, tour vans, shuttles, motorcycles,
            and more — integrated with HostPulse stays and itineraries.
          </p>
          ...
          <div className="mt-10 grid gap-4 md:grid-cols-2">
            ...
            >
              ...
              <p className="mt-2 text-sm text-muted-foreground">
                Register your car-hire business, get verified, then manage your fleet, bookings and
                payouts.
              </p>
            ...
            >
              ...
              <p className="mt-2 text-sm text-muted-foreground">
                Partner with a verified HostPulse rental company. You keep ownership — they manage
                bookings, customers and pricing while you earn.
              </p>
              <div className="mt-3 inline-flex items-center gap-1 text-sm font-medium text-primary">
        ...
        <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
          {(MOBILITY_CATEGORIES as readonly MobilityCategory[]).map((c) => (
            <Link
              ...
              params={{ category: c }}
              aria-label={`Browse ${MOBILITY_CATEGORY_LABELS[c]}`}
            >
              <Card className="h-full transition-all hover:border-primary hover:shadow-md">
                ...
                <CardContent>
                  <p className="text-xs text-muted-foreground">
                    Browse {MOBILITY_CATEGORY_LABELS[c].toLowerCase()} across Kenya.
                  </p>
                </CardContent>
              </Card>
            </Link>
      ...
      <section className="mx-auto max-w-6xl px-6 pb-16">
        ...
        {isLoading ? (
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {vehicles.map((v: any) => {
              const img = (v.mobility_vehicle_images ?? []).sort(
                ...
                <Link key={v.id} to="/mobility/v/$slug" params={{ slug: v.slug }}>
                  <Card className="h-full overflow-hidden transition-all hover:border-primary hover:shadow-md">
                    ...
                    <CardContent className="text-sm text-muted-foreground space-y-1">
                      <div>
                        {MOBILITY_CATEGORY_LABELS[v.category as MobilityCategory] ?? v.category}
                      </div>
                      <div>
                        {v.town ?? v.county_code ?? "Kenya"} · {v.seats ?? "?"} seats ·{" "}
                        {v.transmission ?? "-"}
                      </div>
                      {dayRate && (
                        <div className="font-medium text-foreground">
                          KES {Number(dayRate.price_kes).toLocaleString()} / day
                        </div>
                      )}
                    </CardContent>
                  </Card>
