...
export const Route = createFileRoute("/_authenticated/onboarding")({
  head: () => ({
    meta: authPageMeta({
      ...
      description:
        "Add your hotel, lodge, camp, apartment or villa to HostPulse in under 5 minutes.",
    }),
  }),
  errorComponent: ({ error, reset }) => (
    <DashboardShell>
      <div className="p-12 text-center">
        ...
        <button onClick={reset} className="mt-3 text-primary underline">
          Retry
        </button>
      </div>
    </DashboardShell>
    ...
    return (
      <DashboardShell>
        <EmptyState
          ...
          description="Create or join an organization to start onboarding a property."
        />
      </DashboardShell>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <H
        ...
        desc="Tell us about your property and how guests can reach you."
      />
      <div className="grid gap-4 md:grid-cols-2">
        <F label="Property name *">
          <Input
            ...
            maxLength={120}
            placeholder="e.g. Baobab Beach Resort"
          />
        </F>
...
function Step2Location({ payload, update }: StepProps) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      autoFn({ data: { input: q, sessionToken } })
        .then((r: any) => setSugs(r.suggestions ?? []))
        .catch(() => {});
    }, 300);
  ...
  async function pick(placeId: string) {
    ...
    try {
      ...
      if (d.county && counties.data) {
        const match = counties.data.find(
          (c: any) => c.name.toLowerCase() === d.county.toLowerCase(),
        );
      ...
      <F label="Search on Google Maps">
        <div className="relative">
          <Input
            ...
            placeholder="Search address, hotel or landmark…"
          />
          {sugs.length > 0 && (
...
function Step3Prefill({ payload, update }: StepProps) {
  ...
  async function run() {
    if (!payload.name || payload.name.length < 2) {
      toast.error("Add a property name in step 1 first");
      return;
    }
    setBusy(true);
    try {
      const res = await prefillFn({
        data: {
          name: payload.name,
          location: [payload.town, payload.ward].filter(Boolean).join(", "),
        },
      });
      setResult(res);
    } catch (e: any) {
  ...
  return (
    <div className="space-y-4">
      <H
        title="AI smart prefill"
        desc="We'll suggest public factual info for your property. Nothing is copied from other sites — you review and edit everything before publishing."
      />
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <F label="Number of rooms">
          <Input
            type="number"
            min={0}
            value={payload.numRooms ?? ""}
            onChange={(e) =>
              update({ numRooms: e.target.value === "" ? undefined : Number(e.target.value) })
            }
          />
        </F>
        <F label="Pet policy">
          <Input
            value={payload.petPolicy ?? ""}
            onChange={(e) => update({ petPolicy: e.target.value })}
            maxLength={200}
            placeholder="Pets allowed?"
          />
        </F>
        <F label="Smoking policy">
          <Input
            value={payload.smokingPolicy ?? ""}
            onChange={(e) => update({ smokingPolicy: e.target.value })}
            maxLength={200}
          />
        </F>
      </div>
      ...
      <F label="Accessibility features (comma-separated)">
        <Input
          value={(payload.accessibility ?? []).join(", ")}
          onChange={(e) =>
            update({
...
function Step5Description({ payload, update }: StepProps) {
  ...
  return (
    <div className="space-y-4">
      <H
        title="Description"
        desc="Generate an original SEO-friendly description, then edit as you like."
      />
...
function Step6Media({ payload, update, orgId }: StepProps & { orgId: string }) {
  ...
  async function upload(files: FileList | null, asMain: boolean) {
    ...
    try {
      ...
      for (const file of Array.from(files)) {
        ...
        if (asMain) {
          update({ mainImagePath: path });
          break;
        }
        added.push(path);
      }
      if (added.length)
        update({ galleryPaths: [...(payload.galleryPaths ?? []), ...added].slice(0, 40) });
  ...
  return (
    <div className="space-y-4">
      <H
        title="Photos"
        desc="Upload a cover image and gallery. We compress images automatically."
      />
      ...
      <div className="grid gap-6 md:grid-cols-[1fr_2fr]">
        <div>
          ...
          <MediaPreview
            path={payload.mainImagePath ?? null}
            urlFor={urlFor}
            ...
            {(payload.galleryPaths ?? []).map((path, i) => (
              ...
              >
                ...
                <button
                  ...
                  onClick={() =>
                    update({ galleryPaths: (payload.galleryPaths ?? []).filter((_, j) => j !== i) })
                  }
...
function MediaPreview({
  ...
  useEffect(() => {
    if (path) urlFor(path).then(setUrl);
    else setUrl(null);
  }, [path, urlFor]);
  return (
...
function Step7Rooms({ payload, update }: StepProps) {
  ...
  function rm(i: number) {
    update({ rooms: rooms.filter((_, j) => j !== i) });
  }
  ...
  return (
    <div className="space-y-4">
      <H
        title="Rooms & pricing"
        desc="Add each room category with capacity, pricing and description."
      />
      {rooms.map((r, i) => (
        <div key={i} className="space-y-3 rounded-lg border p-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 text-sm font-medium">
              <Building2 className="h-4 w-4" /> Room {i + 1}
            </div>
            <Button variant="ghost" size="sm" onClick={() => rm(i)}>
          ...
          <div className="grid gap-3 md:grid-cols-3">
            ...
            <F label="Capacity">
              <Input
                ...
                max={64}
                value={r.capacity ?? ""}
                onChange={(e) => set(i, { capacity: Number(e.target.value) })}
              />
            </F>
            <F label="Bed type">
              <Input
                value={r.bedType ?? ""}
                onChange={(e) => set(i, { bedType: e.target.value })}
                placeholder="King, Twin…"
              />
            </F>
            <F label="Price/night (KES)">
              <Input
                type="number"
                min={0}
                value={r.pricePerNight ?? ""}
                onChange={(e) => set(i, { pricePerNight: Number(e.target.value) })}
              />
            </F>
            <F label="Weekend price">
              <Input
                type="number"
                min={0}
                value={r.weekendPrice ?? ""}
                onChange={(e) => set(i, { weekendPrice: Number(e.target.value) })}
              />
            </F>
            <F label="Holiday price">
              <Input
                type="number"
                min={0}
                value={r.holidayPrice ?? ""}
                onChange={(e) => set(i, { holidayPrice: Number(e.target.value) })}
              />
            </F>
          </div>
          <F label="Description">
      ...
      <Button variant="outline" onClick={add}>
        <Plus className="mr-1 h-4 w-4" /> Add room category
      </Button>
    </div>
  );
}
...
function Step8Availability({ payload, update }: StepProps) {
  ...
  return (
    <div className="space-y-6">
      <H
        title="Availability & seasonal pricing"
        desc="Optional — block dates and configure high/low season pricing."
      />
      ...
      <div className="space-y-3">
        ...
        {rates.map((r, i) => (
          <div key={i} className="grid gap-2 md:grid-cols-[1fr_140px_140px_140px_40px]">
            ...
            <Button
              ...
              onClick={() => update({ seasonalRates: rates.filter((_, j) => j !== i) })}
            >
              <Trash2 className="h-4 w-4" />
            </Button>
          </div>
        ...
        <Button
          ...
          onClick={() =>
            update({
              seasonalRates: [...rates, { label: "", startDate: "", endDate: "", price: 0 }],
            })
          }
        >
      ...
      <div className="space-y-3">
        ...
        {blocks.map((b, i) => (
          <div key={i} className="grid gap-2 md:grid-cols-[140px_140px_1fr_40px]">
            ...
            <Button
              ...
              onClick={() => update({ blockedDates: blocks.filter((_, j) => j !== i) })}
            >
              <Trash2 className="h-4 w-4" />
            </Button>
          </div>
        ...
        <Button
          variant="outline"
          size="sm"
          onClick={() => update({ blockedDates: [...blocks, { startDate: "", endDate: "" }] })}
        >
  ...
  return (
    <div className="space-y-4">
      <H
        ...
        desc="Choose how guests can pay. M-Pesa and card payments are wired into HostPulse's payment infrastructure."
      />
      <div className="space-y-3">
        <PayRow
          label="M-Pesa"
          desc="Instant STK push checkout."
          checked={!!p.mpesa}
          onChange={set("mpesa")}
        />
        <PayRow
          label="Visa / Mastercard"
          desc="Card payments via Paddle."
          checked={!!p.cards}
          onChange={set("cards")}
        />
        <PayRow
          label="Bank transfer"
          desc="Manual EFT to your account."
          checked={!!p.bankTransfer}
          onChange={set("bankTransfer")}
        />
        <PayRow
          label="Cash on arrival"
          desc="Guest pays at check-in."
          checked={!!p.cashOnArrival}
          onChange={set("cashOnArrival")}
        />
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-3 rounded-lg border bg-muted/30 p-4 text-sm">
        ...
        <Row
          k="County / town"
          v={`${payload.countyCode ?? "—"} / ${payload.town ?? "—"}${payload.ward ? " / " + payload.ward : ""}`}
        />
        <Row
          k="Coordinates"
          v={
        ...
        <Row
          k="Contact"
          v={[payload.email, payload.phone, payload.whatsapp].filter(Boolean).join(" · ") || "—"}
        />
        <Row k="Amenities" v={(payload.amenities ?? []).join(", ") || "—"} />
        <Row k="Rooms" v={String((payload.rooms ?? []).length)} />
        <Row
          k="Photos"
          v={`${payload.mainImagePath ? "cover set" : "no cover"}, ${(payload.galleryPaths ?? []).length} gallery`}
        />
        <Row
          k="Description"
          v={payload.description ? payload.description.slice(0, 200) + "…" : "—"}
        />
      </div>
      <p className="text-xs text-muted-foreground">
        Your listing goes live once an admin approves it. You can edit anytime from Marketplace →
        your listings.
      </p>
    </div>
  );
