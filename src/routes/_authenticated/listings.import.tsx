...
export const Route = createFileRoute("/_authenticated/listings/import")({
  head: () => ({
    meta: authPageMeta({
      ...
      description: "Bulk import marketplace listings from CSV.",
    }),
  }),
...
function ImportPage() {
  ...
  const parsed = useMemo(() => {
    ...
    try {
      ...
      const missing = HEADERS.filter(
        (h) =>
          !headers.includes(h) &&
          ["name", "category", "countyCode", "town", "description"].includes(h),
      );
      if (missing.length > 0)
  ...
  function downloadSample() {
    ...
    a.href = url;
    a.download = "marketplace-listings-sample.csv";
    a.click();
      ...
      <div className="mx-auto max-w-5xl space-y-6 p-6">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Upload a CSV with up to 200 properties. Listings are created as drafts; submit them for
            review when ready.
          </p>
        </div>
        ...
        <section className="space-y-3 rounded-xl border bg-card p-6">
          ...
          <div className="grid gap-2 md:grid-cols-2 text-xs text-muted-foreground">
            ...
            <div>
              ...
              <code className="block break-words">
                amenities (pipe-separated), pricePerNight, currency, latitude, longitude,
                googleMapsUrl, contactEmail, contactPhone, contactWhatsapp, availability
              </code>
            </div>
          </div>
          <div className="rounded-md bg-muted/30 p-3 text-xs">
            <p>
              ...
              {PROPERTY_CATEGORIES.map((c) => c.value).join(", ")}
            </p>
            <p className="mt-1">
              <strong>County code</strong> — e.g.{" "}
              {counties.data
                ?.slice(0, 4)
                .map((c) => c.code)
                .join(", ") || "KE-NRB, KE-MSA…"}
          ...
          <div className="flex items-center justify-between border-t pt-4">
            <p className="text-sm text-muted-foreground">
              {parsed.rows.length > 0
                ? `${parsed.rows.length} rows ready to import.`
                : "Add some rows to enable import."}
            </p>
            <Button
              onClick={() => run.mutate()}
