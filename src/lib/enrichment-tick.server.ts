...
const ROOM_TYPES = ["bedroom","bathroom","kitchen","living_room","dining","exterior","pool","garden","view","balcony","office","reception","gym","spa","restaurant","other"];
  ...
  for (const row of pool) {
    try {
      const out = await aiVisionJSON<any>({
        ...
        prompt: "Analyze this listing photo. Identify room type, notable features, dominant colors, a 1-sentence caption, a quality score 0–100, and safety flags such as 'watermark','stock_photo','low_resolution','inappropriate'.",
...
const ASPECTS = ["cleanliness","host","value","location","comfort","amenities","accuracy","communication"];
const ReviewSchema = {
  ...
  schema: {
    ...
    properties: {
      ...
      aspects: { type: "object", properties: Object.fromEntries(ASPECTS.map((a) => [a, { type: "number" }])) },
...
export async function runReviewNlpTick(limit = 20): Promise<{ processed: number; attempted: number }> {
  ...
  for (const r of rows ?? []) {
    ...
    try {
      const out = await aiJSON<any>({
        system: "You analyze short-stay guest reviews. Return only JSON. sentiment in [-1,1]; aspect scores in [-1,1] (0 = not mentioned).",
