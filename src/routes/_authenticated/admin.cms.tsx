    ...
    <div className="mx-auto max-w-7xl space-y-6 p-6">
      <header>
        ...
        <p className="text-sm text-muted-foreground">
          Counties, discovery sources, reviews, and promo content.
        </p>
      </header>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm text-muted-foreground">Counties</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-2xl font-semibold">{data?.counties.length ?? 0}</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm text-muted-foreground">Discovery sources</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-2xl font-semibold">{data?.sources.length ?? 0}</p>
            <p className="mt-1 text-xs text-muted-foreground">
              {(data?.sources ?? []).filter((x: any) => x.active).length} active
            </p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm text-muted-foreground">Discovered pending</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-2xl font-semibold text-amber-500">{data?.discovery.pending ?? 0}</p>
            <p className="mt-1 text-xs text-muted-foreground">
              {data?.discovery.published ?? 0} published
            </p>
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent>
            <p className="text-2xl font-semibold">
              {(data?.coupons ?? []).filter((c: any) => c.active).length}
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between">
            <CardTitle className="flex items-center gap-2">
              <Compass className="h-4 w-4" /> Discovery sources
            </CardTitle>
            <Button asChild size="sm" variant="outline">
              <Link to="/listings/admin/discovery">Manage</Link>
            </Button>
          </CardHeader>
              ...
              {(data?.sources ?? []).map((src: any) => (
                <li key={src.id} className="flex items-center justify-between p-3 text-sm">
                  ...
                  <span className="text-xs text-muted-foreground">
                    ...
                    {src.last_run_at ? new Date(src.last_run_at).toLocaleDateString() : "never"}
                  </span>
                </li>
              ))}
              {(data?.sources ?? []).length === 0 && !isLoading && (
                <li className="p-4">
                  <EmptyState
                    ...
                    description="Add a discovery source to start crawling listings."
                  />
                </li>
              )}
            </ul>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <MapPin className="h-4 w-4" /> Counties
            </CardTitle>
          </CardHeader>
          <CardContent className="p-0">
            <ul className="divide-y max-h-96 overflow-auto">
      ...
      <Card>
        ...
        <CardContent className="p-0">
          <ul className="divide-y">
            ...
            {(data?.recentReviews ?? []).length === 0 && !isLoading && (
              <li className="p-4">
                <EmptyState
                  title="No reviews yet"
                  description="Guest reviews will appear here once submitted."
                />
              </li>
            )}
          </ul>
