...
import {
  getMyMobilityVehicle,
  upsertMobilityVehicle,
  submitMobilityVehicle,
  archiveMobilityVehicle,
  deleteMobilityVehicle,
  setMobilityVehicleRates,
  upsertMobilitySeasonalRate,
  deleteMobilitySeasonalRate,
  addMobilityVehicleImage,
  deleteMobilityVehicleImage,
  blockMobilityDates,
  unblockMobilityDates,
  listMobilityProviderBookings,
  respondMobilityBooking,
  MOBILITY_CATEGORIES,
        ...
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <div className="mt-1 flex items-center gap-2 text-sm text-muted-foreground">
              ...
              <span>
                · {(MOBILITY_CATEGORY_LABELS as Record<string, string>)[v.category] ?? v.category}
              </span>
            </div>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Button
              ...
              variant="outline"
              onClick={() =>
                archive({ data: { id, archived: !v.is_archived } })
                  .then(() => {
                    toast.success(v.is_archived ? "Unarchived" : "Archived");
                    invalidate();
                  })
                  .catch((e: any) => toast.error(e?.message ?? "Failed"))
              }
        ...
        <Tabs defaultValue="details">
          <TabsList className="flex-wrap">
            ...
            <TabsTrigger value="images">
              <ImageIcon className="mr-1 h-3.5 w-3.5" /> Images
            </TabsTrigger>
            <TabsTrigger value="rates">
              <DollarSign className="mr-1 h-3.5 w-3.5" /> Rates
            </TabsTrigger>
            <TabsTrigger value="seasonal">Seasonal</TabsTrigger>
            <TabsTrigger value="tiers">
              <Tag className="mr-1 h-3.5 w-3.5" /> Tiers
            </TabsTrigger>
            ...
            <TabsTrigger value="calendar">
              <Calendar className="mr-1 h-3.5 w-3.5" /> Availability
            </TabsTrigger>
            <TabsTrigger value="bookings">
              <MessageSquare className="mr-1 h-3.5 w-3.5" /> Bookings
            </TabsTrigger>
          ...
          <TabsContent value="seasonal">
            <SeasonalTab
              vehicleId={id}
              rates={q.data?.seasonalRates ?? []}
              upsert={upsertSeasonal}
              remove={deleteSeasonal}
              onChanged={invalidate}
            />
          </TabsContent>
          ...
          <TabsContent value="bookings">
            <BookingsTab
              bookings={bookingsQ.data?.bookings ?? []}
              isLoading={bookingsQ.isLoading}
              respond={respond}
              onChanged={() => qc.invalidateQueries({ queryKey: ["mobility-provider-bookings"] })}
            />
          </TabsContent>
...
function DetailsTab({ v, upsert, onSaved }: any) {
  const [f, setF] = useState<any>({
    ...
    mileagePolicy: v.mileage_policy ?? "",
    fuelPolicy: v.fuel_policy ?? "",
    licenseRequirements: v.license_requirements ?? "",
    mainImageUrl: v.main_image_url ?? "",
    description: v.description ?? "",
    hasAc: v.has_ac ?? true,
    hasGps: v.has_gps ?? false,
    hasBluetooth: v.has_bluetooth ?? false,
    hasChildSeat: v.has_child_seat ?? false,
    isLuxury: v.is_luxury ?? false,
    isElectric: v.is_electric ?? false,
    isHybrid: v.is_hybrid ?? false,
    isWedding: v.is_wedding ?? false,
    isSafari: v.is_safari ?? false,
    ...
    mutationFn: () =>
      upsert({
        data: {
          ...
          countyCode: f.countyCode || undefined,
    ...
    <Card>
      ...
      <CardContent className="grid gap-3 sm:grid-cols-3">
        <Sel
          label="Category"
          value={f.category}
          onChange={(x) => setF({ ...f, category: x })}
          options={MOBILITY_CATEGORIES.map((c) => [c, MOBILITY_CATEGORY_LABELS[c]])}
        />
        <Sel
          label="Body style"
          value={f.vehicleType}
          onChange={(x) => setF({ ...f, vehicleType: x })}
          options={MOBILITY_VEHICLE_TYPES.map((t) => [t, MOBILITY_VEHICLE_TYPE_LABELS?.[t] ?? t])}
        />
        ...
        <F
          ...
          on={(x) => setF({ ...f, registrationPlate: x })}
        />
        <Sel
          label="Transmission"
          value={f.transmission}
          onChange={(x) => setF({ ...f, transmission: x })}
          options={
            [
              ["automatic", "Automatic"],
              ["manual", "Manual"],
        ...
        <F
          ...
          on={(x) => setF({ ...f, minDriverAge: x })}
        />
        <F
          label="Security deposit (KES)"
          type="number"
          value={f.securityDepositKes}
          on={(x) => setF({ ...f, securityDepositKes: x })}
        />
        <F
          label="Promo daily (KES)"
          type="number"
          value={f.promoPriceKes}
          on={(x) => setF({ ...f, promoPriceKes: x })}
        />
        ...
        <div className="sm:col-span-3">
          ...
          <Input
            value={f.mainImageUrl}
            onChange={(e) => setF({ ...f, mainImageUrl: e.target.value })}
            placeholder="https://…"
          />
        </div>
        <div className="sm:col-span-3">
      ...
      <CardContent className="space-y-4">
        <div className="grid gap-2 sm:grid-cols-[1fr_1fr_auto]">
          ...
          <Button
            disabled={!url}
            onClick={() =>
              addImg({ data: { vehicleId, url, alt, sortOrder: images.length } })
                .then(() => {
                  toast.success("Added");
                  setUrl("");
                  setAlt("");
        ...
        {images.length === 0 ? (
          <EmptyState
            title="No images yet"
            description="Paste image URLs from your website or CDN."
          />
        ) : (
...
function RatesTab({ vehicleId, rates, save, onSaved }: any) {
  const initial = useMemo(() => {
    ...
    for (const u of RATE_UNITS) {
      ...
      map[u] = {
        ...
        includedKm: r?.included_km ?? "",
        extraKmKes: r?.extra_km_kes ?? "",
      };
    }
    return map;
  }, [rates]);
  ...
  const commit = useMutation({
    mutationFn: () => {
      const payload = RATE_UNITS.filter(
        (u) => state[u].priceKes !== "" && Number(state[u].priceKes) > 0,
      ).map((u) => ({
        unit: u,
    ...
    <Card>
      ...
      <CardContent className="space-y-3">
        <p className="text-xs text-muted-foreground">
          Set at least a daily rate. Leave others blank to disable that rental unit.
        </p>
          ...
          {RATE_UNITS.map((u) => (
            <div key={u} className="grid gap-2 rounded-md border p-3 sm:grid-cols-5">
              ...
              <F
                label="Price (KES)"
                type="number"
                value={String(state[u].priceKes)}
                on={(x) => setState({ ...state, [u]: { ...state[u], priceKes: x } })}
              />
              <F
                label="Min units"
                type="number"
                value={String(state[u].minUnits)}
                on={(x) => setState({ ...state, [u]: { ...state[u], minUnits: x } })}
              />
              <F
                label="Included km"
                type="number"
                value={String(state[u].includedKm)}
                on={(x) => setState({ ...state, [u]: { ...state[u], includedKm: x } })}
              />
              <F
                label="Extra km (KES)"
                type="number"
                value={String(state[u].extraKmKes)}
                on={(x) => setState({ ...state, [u]: { ...state[u], extraKmKes: x } })}
              />
            </div>
          ))}
        </div>
        <Button onClick={() => commit.mutate()} disabled={commit.isPending}>
          {commit.isPending ? "Saving…" : "Save pricing"}
        </Button>
      </CardContent>
    </Card>
  );
...
function SeasonalTab({ vehicleId, rates, upsert, remove, onChanged }: any) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Seasonal & promotional rates</CardTitle>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="grid gap-2 sm:grid-cols-6">
          ...
          <F
            ...
            type="date"
            value={f.startsOn}
            on={(x) => setF({ ...f, startsOn: x })}
          />
          <F label="Ends" type="date" value={f.endsOn} on={(x) => setF({ ...f, endsOn: x })} />
          <Sel
            label="Unit"
            value={f.unit}
            onChange={(x) => setF({ ...f, unit: x as RateUnit })}
            options={RATE_UNITS.map((u) => [u, u]) as any}
          />
          <F
            label="Price (KES)"
            type="number"
            value={f.priceKes}
            on={(x) => setF({ ...f, priceKes: x })}
          />
          <F label="Promo code" value={f.promoCode} on={(x) => setF({ ...f, promoCode: x })} />
        </div>
        <Button
          disabled={!f.label || !f.startsOn || !f.endsOn || !f.priceKes}
          onClick={() =>
            upsert({
              data: {
                vehicleId,
        ...
        {rates.length === 0 ? (
          <EmptyState
            title="No seasonal rates"
            description="Create high-season, holiday, or promo pricing."
          />
        ) : (
              ...
              <div key={r.id} className="flex items-center justify-between rounded-md border p-3">
                <div>
                  <div className="font-medium">
                    {r.label} · KES {Number(r.price_kes).toLocaleString()}/{r.unit}
                  </div>
                  <div className="text-xs text-muted-foreground">
                    {r.starts_on} → {r.ends_on}
                    {r.promo_code ? ` · code ${r.promo_code}` : ""}
                  </div>
                </div>
                <Button
                  size="sm"
...
function AvailabilityTab({ vehicleId, blocks, block, unblock, onChanged }: any) {
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        <p className="text-xs text-muted-foreground">
          Block dates for maintenance or off-market periods. Booking holds appear here
          automatically.
        </p>
              ...
              onClick={() =>
                block({
                  ...
                  .then(() => {
                    ...
                    setF({ startAt: "", endAt: "", reason: "maintenance" });
                    onChanged();
                  })
                  .catch((e: any) => toast.error(e?.message ?? "Failed"))
              }
        ...
        {blocks.length === 0 ? (
          <EmptyState title="No blocks" description="Vehicle is fully available." />
        ) : (
          <div className="space-y-2">
            {blocks.map((b: any) => (
  ...
  if (bookings.length === 0)
    return (
      <EmptyState
        ...
        description="Reservations will appear here as they come in."
      />
    );
  return (
    <Card>
            ...
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div>
                <div className="font-medium">
                  ...
                  {new Date(b.dropoff_at).toLocaleString()}
                </div>
                <div className="text-xs text-muted-foreground">
                  {b.guest_name ?? "Guest"} · KES {Number(b.total_kes ?? 0).toLocaleString()} ·{" "}
                  {b.driver_option ?? "self"} · {b.payment_status}
                </div>
              </div>
              <Badge
                variant={
            ...
            {b.status === "pending" && (
              <div className="mt-2 flex gap-2">
                <Button
                  size="sm"
                  onClick={() =>
                    respond({ data: { id: b.id, status: "confirmed" } })
                      .then(() => {
                ...
                <Button
                  ...
                  variant="outline"
                  onClick={() =>
                    respond({ data: { id: b.id, status: "declined", message: "Unavailable" } })
                      .then(() => {
            ...
            {b.status === "confirmed" && (
              <div className="mt-2 flex gap-2">
                <Button
                  ...
                  variant="outline"
                  onClick={() =>
                    respond({ data: { id: b.id, status: "completed" } }).then(() => {
                ...
                <Button
                  ...
                  variant="ghost"
                  onClick={() =>
                    respond({ data: { id: b.id, status: "cancelled" } }).then(() => {
...
function TiersTab({ vehicleId, orgId }: { vehicleId: string; orgId: string }) {
  ...
  const q = useQuery({
    queryKey: ["mobility-tiers", vehicleId],
    queryFn: () => list({ data: { vehicleId } }),
  });
  const [f, setF] = useState({
    tier: "daily",
    priceKes: "",
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        <p className="text-xs text-muted-foreground">
          Layer on tiered pricing beyond the base rates — corporate deals, weekend surcharges, lease
          rates.
        </p>
        <div className="grid gap-2 sm:grid-cols-6">
          <Sel
        ...
        {q.isLoading ? (
          ...
          <EmptyState
            ...
            description="Add tiered pricing to unlock corporate & long-term rates."
          />
        ) : (
          <div className="space-y-2">
            {rows.map((r) => (
              <div key={r.id} className="flex items-center justify-between rounded-md border p-3">
                <div>
                  <div className="font-medium capitalize">
                    {r.tier} · KES {Number(r.price_kes).toLocaleString()}{" "}
                    <span className="text-xs text-muted-foreground">/ min {r.min_units}</span>
                  </div>
                  <div className="text-xs text-muted-foreground">
...
function DocumentsTab({ vehicleId, orgId }: { vehicleId: string; orgId: string }) {
  ...
  const q = useQuery({
    queryKey: ["mobility-docs", vehicleId],
    queryFn: () => list({ data: { vehicleId } }),
  });
  const [f, setF] = useState({
    docType: "insurance",
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        <p className="text-xs text-muted-foreground">
          Upload insurance, inspection, and logbook records. Expiring documents get flagged.
        </p>
        <div className="grid gap-2 sm:grid-cols-6">
          <Sel
            label="Type"
            value={f.docType}
            onChange={(x) => setF({ ...f, docType: x })}
            options={DOC_TYPES}
          />
          ...
          <F
            ...
            type="date"
            value={f.issuedAt}
            on={(x) => setF({ ...f, issuedAt: x })}
          />
          <F
            label="Expires"
            type="date"
            value={f.expiresAt}
            on={(x) => setF({ ...f, expiresAt: x })}
          />
        ...
        {q.isLoading ? (
          ...
          <EmptyState
            title="No documents"
            description="Add compliance records to speed up approval."
          />
        ) : (
          <div className="space-y-2">
            {rows.map((r) => {
                  ...
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      {!expired && expiringSoon && (
                        <Badge variant="secondary">Expires in {d}d</Badge>
                      )}
                    </div>
                    <div className="text-xs text-muted-foreground">
...
function MaintenanceTab({ vehicleId, orgId }: { vehicleId: string; orgId: string }) {
  ...
  const q = useQuery({
    queryKey: ["mobility-maintenance", vehicleId],
    queryFn: () => list({ data: { vehicleId } }),
  });
  const [f, setF] = useState({
    maintenanceType: "",
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        <p className="text-xs text-muted-foreground">
          Track service history, upcoming maintenance, and repair costs.
        </p>
        <div className="grid gap-2 sm:grid-cols-4">
          <F
            label="Type"
            value={f.maintenanceType}
            on={(x) => setF({ ...f, maintenanceType: x })}
          />
          ...
          <F
            ...
            type="datetime-local"
            value={f.scheduledAt}
            on={(x) => setF({ ...f, scheduledAt: x })}
          />
          <F
            label="Completed"
            type="datetime-local"
            value={f.doneAt}
            on={(x) => setF({ ...f, doneAt: x })}
          />
          <F
            label="Cost (KES)"
            type="number"
            value={f.costKes}
            on={(x) => setF({ ...f, costKes: x })}
          />
          <F
            label="Odometer (km)"
            type="number"
            value={f.odometerKm}
            on={(x) => setF({ ...f, odometerKm: x })}
          />
          <Sel
            label="Status"
            value={f.status}
            onChange={(x) => setF({ ...f, status: x })}
            options={MAINT_STATUS}
          />
        ...
        {q.isLoading ? (
          ...
          <EmptyState
            ...
            description="Log services to keep this vehicle roadworthy."
          />
        ) : (
          <div className="space-y-2">
            {rows.map((r) => (
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {r.cost_kes ? (
                    <span className="text-xs text-muted-foreground">
                      KES {Number(r.cost_kes).toLocaleString()}
                    </span>
                  ) : null}
                </div>
                <div className="text-xs text-muted-foreground">
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        {q.isLoading ? (
          ...
          <EmptyState
            title="No reviews"
            description="Guest reviews will appear here for moderation."
          />
        ) : (
              ...
              <div key={r.id} className="rounded-md border p-3 space-y-2">
                <div className="flex flex-wrap items-center gap-2">
                  <div className="flex items-center gap-1 font-medium">
                    {Array.from({ length: 5 }).map((_, i) => (
                      <Star
                        key={i}
                        className={`h-3.5 w-3.5 ${i < Number(r.rating ?? 0) ? "fill-yellow-500 text-yellow-500" : "text-muted-foreground"}`}
                      />
                    ))}
                  </div>
                  <Badge
                ...
                {r.provider_response && (
                  <div className="rounded bg-muted p-2 text-xs">
                    <span className="font-medium">Your reply:</span> {r.provider_response}
                  </div>
                )}
                {r.status === "pending" && (
                  <div className="space-y-2">
                    <Textarea
                      rows={2}
                      placeholder="Optional provider response…"
                      value={responses[r.id] ?? ""}
                      onChange={(e) => setResponses({ ...responses, [r.id]: e.target.value })}
                    />
                    <div className="flex gap-2">
                      <Button size="sm" onClick={() => act(r.id, "approve", responses[r.id])}>
